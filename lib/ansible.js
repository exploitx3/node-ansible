...
var Playbook = function () {
  ...
  this.addExtraCommandArgument = function (argument) {
    this.config.extraArguments.push(argument)
    return this
  }
  ...
  this.compileParams = function() {
    ...
    if (this.config.extraArguments.length !== 0) {
      var extrArgs =  this.config.extraArguments.join(' ')
      result = result.concat(extrArgs)
    }
